...
// `bugcrunch_state`, but the push to `sdk.data` only fires after the persist (~200ms)
...
// drain synchronously-as-possible: write `bugcrunch_state` now → SaveManager proxy →
...
describe('flushSaveNow — immediate flush on a hard checkpoint', () => {
  it('pushes a pending level write to the backend without waiting for the debounce', async () => {
    ...
    // A cleared level writes the new best into bugcrunch_state (still sitting on the
    // debounce timers — nothing has reached the cloud yet).
    setState('bc_best_level', 2)
    ...
    expect(cloudBlob.bc_best_level).toBe(2)
  })
  ...
  it('also carries coexisting progress (coins) written in the same checkpoint', async () => {
    ...
    setState('bc_coins', 250)
    setState('bc_best_level', 3)
    await flushSaveNow()

    const cloudBlob = JSON.parse(data.store.get(STATE_KEY) || '{}')
    expect(cloudBlob.bc_best_level).toBe(3)
    expect(cloudBlob.bc_coins).toBe(250)
  })
})
...
describe('discrete progression events flush to the backend immediately', () => {
  it('buying a shoe flushes without waiting for the debounce', async () => {
    ...
    expect(blob.bc_shoes_owned).toContain('steelBoot')
    expect(blob.bc_shoe).toBe('steelBoot')
  })
