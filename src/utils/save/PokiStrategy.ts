...
//      detect. bug-crunch's `bugcrunch_state` blob is well inside that today
...
//      SDK: poki_evensx_user_id, poki_pbf, poki_uid*, poki_session, poki_source,
//      poki_pubcid, poki_no_ads, poki_url. This project's keys are `bugcrunch_state`
//      / `bc_*` / `__save_*`, so there is no collision — keep it that way.
//
