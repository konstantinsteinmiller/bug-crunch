...
//      detect. splatix's `splatix_state` blob is well inside that today
...
//      poki_pubcid, poki_no_ads, poki_url. This project's keys are `splatix_state`
//      / `sx_*` / `__save_*`, so there is no collision — keep it that way.
//
