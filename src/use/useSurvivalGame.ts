...
export const attackIncoming = (): boolean => {
  ...
  // supposed to trust on the swing that follows — so the cheapest honest thing
  // is to not raise it. The health bar going UP, with its own green burst and a
  // rising plus, is the heal's tell.
  //
 ...
 * the moment the boss pool opens: a healer's third cast raises a wind-up nobody
 * can step out of.
 *
 ...
 * and one more i18n key beside `hud.dodge` (`hud.burst`, say, or `hud.healing`)
 * chosen from it. Nothing else changes: the badge's position, its animation and
 * its `hud.incoming` aria-label are all correct for both. That component is not
 * in this change's remit, so the state is exported and the wiring is not done.
 *
 ...
 */
export type IncomingKind = 'slam' | 'rake' | 'bolt' | 'heal' | 'sweep'
...
export interface Incoming {
   ...
   * half-truth in the badge's wording rather than one the boss pool introduced —
...
export const incomingThreat = (): Incoming | null => {
  ...
  const e = foes.find((f) => f.elite && !f.dead && f.sweepCd > 0 && f.sweepCd <= ELITE_TELEGRAPH)
  return e ? { kind: 'sweep', dodgeable: false, ttl: e.sweepCd } : null
}
