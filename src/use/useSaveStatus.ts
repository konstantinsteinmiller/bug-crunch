...
import { reloadBugCrunchState, flushPersist } from '@/use/useBugCrunchState'
...
/**
 * Refresh the in-memory `bugcrunch_state` blob from (now-patched) localStorage,
 ...
 * and `getState` reads from the `bugCrunchState` ref — so the blob MUST already
 ...
 *   1. `flushPersist()` writes the in-memory `bugcrunch_state` blob to localStorage
