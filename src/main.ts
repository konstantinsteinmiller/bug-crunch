...
import { hasState, reloadBugCrunchState } from '@/use/useBugCrunchState'
...
const bootstrap = async () => {
  ...
  //
  // Why this matters: bug-crunch's main bundle ballooned by ~250KB
  ...
  if (
    ...
    // vConsole removed from this project (was a bug-crunch native-build
  ...
  // backend. CG QA explicitly requires that no `bugcrunch_state` / `bc_*` /
  ...
  // Defense-in-depth `bugcrunch_state` / `bc_*` / `__save_*` safety remove on
  ...
  if (import.meta.env.VITE_APP_CRAZY_WEB === 'true') {
    try {
      ...
      for (let i = 0; i < window.localStorage.length; i++) {
        ...
        if (k && (k === 'bugcrunch_state' || k.startsWith('bc_') || k.startsWith('__save_'))) {
          stragglers.push(k)
        }
      }
  ...
  // Refresh the in-memory `bugCrunchState` blob from the hydrated localStorage so
  ...
  // `reloadBugCrunchState()` defensively, so this is the early-flush companion, not
  // a replacement.
  reloadBugCrunchState()
  ...
  // `bc_user_language` means "this player has never picked a
  ...
  // null/non-null probe here is a reliable signal.
  {
    ...
    stopLangSync = watch(
      ...
      (ready) => {
        if (!ready) return
        stopLangSync?.()
        // Defensive reload — `main.ts` already calls `reloadBugCrunchState()`
